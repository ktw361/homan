...
class Fitter(object):
    def __init__(self,
        ...
        if interpolation_dir is not None:
            self.mask_extractor = InterpolatedMaskExtractor()
        else:
...
def main(args):
    ...
    image_size = 640
    dataset = EpicFrame(mode=args.dataset_mode,
                        frame_nb=1,
                        frame_step=args.chunk_step,
                        use_cache=False,
                        hdf5_root=args.epic_hdf5_root,
    ...
    for sample_idx in range(args.data_offset, len(dataset), args.data_step):
        ...
        try:
            ...
            logging.error(f"IndexError occurs at sample_idx = {sample_idx}.")
            import pudb
            pudb.set_trace()
            continue
