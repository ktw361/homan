...
ARCTIC_IMAGES_DIR = './local_data/arctic_data/cropped_images'
...
class SeqReader:
    def __init__(self, 
        ...
        self.ioi_offset = io.read_json(
            ('./local_data/arctic_data/meta/misc.json')
            )[sid]['ioi_offset']
        ...
        mesh = trimesh.load(
            (f'./local_data/arctic_data/meta/object_vtemplates/{obj_name}/mesh.obj'))
        self.vo_orig = torch.from_numpy(mesh.vertices).float() / 1000
        parts_ids = io.read_json(
            (f'./local_data/arctic_data/meta/object_vtemplates/{obj_name}/parts.json'))
        PART_TOP_ID = 0
    ...
    def use_preprocessed_verts(self, frame_idx):
        ...
        verts as output by running e.g.
            python scripts_data/process_seqs.py \
                ...
                --export_verts
        """
        # raise DeprecationWarning("Need to run arctic code to exporting verts")
