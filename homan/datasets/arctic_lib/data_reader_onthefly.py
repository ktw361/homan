...
from homan.datasets.arctic_lib.arctic_obj_loader import ArcticOBJLoader
...
RAW_SEQS_DIR = to_absolute_path('./local_data/arctic_data/raw_seqs')
...
ARCTIC_OUT_DIR = to_absolute_path('./local_data/arctic_outputs')
...
class SeqReaderOnTheFly:
    def __init__(self,
        ...
        self.ioi_offset = io.read_json(
            to_absolute_path('./local_data/arctic_data/meta/misc.json')
            )[sid]['ioi_offset']
        ...
        self.l_mano_layer = ManoLayer(
            ...
            mano_root=to_absolute_path('./extra_data/mano/'))
        ...
        self.obj_diameter = io.read_json(
            to_absolute_path('./local_data/arctic_data/meta/object_meta.json'))[obj_name]['diameter']
        ...
        self.K_ego = torch.as_tensor(egocam['intrinsics']).view(1, 3, 3).repeat(self.num_frames, 1, 1)
    ...
    def hand_tips(self, side, frame_idx=None, space='ego'):
        ...
        def forward_hand_verts_myimpl(side):
            ...
            shape_h = torch.from_numpy(self.mano_params[side]['shape']).view(1, 10).repeat(self.num_frames, 1)
    ...
    def neutralized_obj_params(self, debug=False):
        ...
        vo_neutral = pose_apply(T_neutral.repeat(len(vo), 1, 1), vo)
        if debug:
    ...
    def pose_hand2world(self, side):
        ...
        _, _, T_world = ManoLayerTracer(
            flat_hand_mean=False, ncomps=45, side=side, use_pca=False,
            mano_root=to_absolute_path('./extra_data/mano/')).forward_transform(
            torch.cat([rot_h, pose_h], axis=-1),
            th_betas=shape_h, th_trans=trans_h, root_palm=True)
        return T_world
