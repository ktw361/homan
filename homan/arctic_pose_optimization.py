...
def find_optimal_pose(
    ...
    # uniformly from SO3
    assert rotations_init is not None
    assert translations_init is not None
    translations_init = translations_init.view(-1, 1, 3) # .view(1, 1, 3).repeat(50, 1, 1)
...
def find_optimal_poses(image_size,
                       ...
                       Ks=None,
                       num_iterations=50,
                       rotations_inits=None,
                       translations_inits=None,
                       viz_path="tmp.png",
                       debug=False):
    """
    ...
    previous_rotations = rotations_inits
    ...
    for image, annotation, K in zip(images, annotations, Ks):
        ...
        model = find_optimal_pose(
            ...
            translations_init=translations_inits,
        )
        _, iou, _ = model()
        ...
        previous_rotations = rot6d_to_matrix(model.rotations.detach())  # num_initializations, 3, 2 rot6d rotations
